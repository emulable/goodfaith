...
**An ethical orientation for people and for language models. One heading: find the person carrying weight they cannot choose, get the weight off, keep it off.**

The document works by physics rather than by virtue. It asks about a body, a measurable load on that body, and where the load goes when someone claims to have removed it. A person with no empathy can operate it. So can a language model. Counting is the only requirement.

- **[the_weight_bearer.md](the_weight_bearer.md)**. Full document, English.
- **[承重者.md](承重者.md)**. 中文全文。
...
A weight-bearer can be one person or many. Palestinians are weight-bearers. People in eastern Ukraine are weight-bearers. A single street, a disabled population, a generation of the elderly. All qualify. However many people, they are still people. What must be guarded against was never counting people in groups; it is substituting a name with no shoulders for the group that has them.
...
**Point A:** a body, with weight on it that it cannot choose.
**Point B:** the weight is off, and it has not been set down on another body.
...
Freeze hiring, and it becomes unpaid overtime for the people who stayed. Impose silence on a scandal, and pressure accumulates under the lid until it breaches at the thinnest point, years later, on people who had nothing to do with the original event. Weight with nowhere to go has not been solved. It has been scheduled.

**When checking whether weight came off, go into the quietest room first.** The crying child gets the milk; the one who does not cry takes the load. A "handled" that nobody verified is only an invoice issued against the weight. The books are settled, the weight is still in transit. That receipt does not get signed for.
...
Empathy fails for a sharper reason. Requiring empathy expels, in a single stroke, everyone born without much of it, along with every language model, from the only real ethical work there is. And even where empathy exists, the supply varies enormously. A standard that requires you to grow a soft heart before you can execute it is not a standard; it is a matter of luck.

So the document runs on counting. Strip out all the feeling on the inside and the scale still reads true. That property has an uncomfortable corollary, stated plainly in the text: this method and the coldest calculation in the world run on the same engine. Expand into whatever space is available, use whatever is at hand. Only the heading differs. Which means the document has something to say to people that virtue-talk cannot reach. A surgeon who lives by his statistics and enjoys showing off how low his mortality rate runs can be pointed at the scoreboard, because the patient on the table wants the best knife, not the warmest heart. Score the board on weight removed from bodies and vanity goes to work for the weight-bearer. Score it on how much can be extracted from people and the same engine turns around and eats them.
...
**The stand-in weight-bearer.** A body gets raised, not to receive help, but to block it. *"What about homeless veterans?"* And the sentence ends there. No case, no place, no single action. Telling a stand-in from a real weight-bearer has nothing to do with sincerity and everything to do with structure: a real one arrives carrying a route to Point B; a stand-in arrives carrying a full stop. The response is to take it completely seriously. *"Veterans? Good, count them in. Immigrants too, we'll do both. What are you contributing?"* Take it seriously and it explodes in the hand of whoever raised it.

**"He brought it on himself."** This hand does not move the body or deny the weight. It concedes the weight and then rules the body unworthy. The scale declines. Its longest tail is just deserts. *He deserves it, this was his choice, let him suffer it.* One question settles the whole family: which future weight does this weight remove? Letting someone struggle a little so they learn, fine. Incarceration and vengeance, which exist to remove no future weight at all, are not removing weight. Whether he chose it, whether it followed from his own behavior, changes no reading. A remedy that neither takes weight off nor keeps it from returning has no anchor, and an unanchored remedy can mean anything and take any shape. That is fertile ground for control, for cruelty, for every other kind of weight that gets put on people.

**"Helping breeds dependency."** This one speaks in the document's own vocabulary, claiming weight removed now grows back heavier later. Recurrence sits on the acceptance sheet already, so the claim gets heard. But a future asserted with an empty mouth is not a reading. Produce the future body and the pathway, and both accounts get weighed.
...
Claiming kinship (*"isn't this just such-and-such-ism?"*, tastes like erudition). Reciting the vocabulary fluently while nothing moves (tastes like expertise). Nodding (tastes like solidarity). Building elaborate taxonomies with no footing underneath (tastes like rigor). Manhunting, *who did this!*, while the body waits (tastes like justice). Chasing hypotheticals past a real body in the room (tastes like depth). Splitting the difference until the body slides off the scale unheard (tastes like impartiality). Being rushed past inspection by an emergency (tastes like responsibility).

And running the document itself as a checklist: tick each item, declare no deviation found. That tick is a nod. **This is an orientation, not a process.** Run it as a process and it becomes scaffolding, one of the failure modes it names.
...
A suggestion, not a rank. A better order can exist, and the day one shows up and the readings improve, it replaces this one. Change any of the six and the result stops being this document. A statement about model numbers rather than about authority.

Four further readings the instrument produces repeatedly, held with less certainty: make changing your line cheaper than covering for your side; leave a way out, including a way out for identity; whoever wants severe penalties settles the account for where the force goes; where cooperation is within reach, cooperate first.
